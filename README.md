...
| Pointers       | `&T{…}` / `&x` (a no-copy reference — go-rs composite values are heap handles), `*p` deref, `new(T)` (a pointer to a zero value of `T`); `f(&x)` and a pointer-receiver method share the pointed-to struct, and `==` on an allocated pointer compares **identity** (two `errors.New("x")` are distinct) while struct values still compare field by field. **A `*T` held in a variable is copied when it is bound again** — `q := p`, `f(p)`, a store into a container — because the static type a copy is decided from has no pointer in it; writes through the second name are then invisible to the first (BUGS.md, "A `*T` is copied at every bind site") |
