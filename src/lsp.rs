...
const CORPUS: &[Entry] = &[
    ...
    e(
        ...
        "Whether the code point `r` occurs in `s`.",
        "strings.ContainsRune(\"héllo\", 'é')   // true",
    ),
    e(
        "strings.ContainsAny",
        "Package strings",
        "strings.ContainsAny(s, chars string) bool",
        "Whether any code point of `chars` occurs in `s`. An empty `chars` is false.",
    ...
    e(
        ...
        "The index of the first *byte* equal to `c`, or -1. Searching by byte finds a byte inside a multi-byte rune, which a rune search would step over.",
        "strings.IndexByte(\"golang\", 'l')   // 2",
    ),
    e(
        "strings.IndexRune",
        "Package strings",
        "strings.IndexRune(s string, r rune) int",
        "The *byte* index where the code point `r` starts, or -1.",
    ...
    e(
        ...
        "The byte index of the first code point of `s` that appears in `chars`, or -1.",
    ...
    e(
        ...
        "The index of the last *byte* equal to `c`, or -1.",
    ...
    e(
        ...
        "Like `Split`, but at most `n` parts — the last holds the unsplit remainder. `n == 0` is a nil slice and `n < 0` means no limit. An empty `sep` splits into runes, `n-1` of them followed by the rest of the string whole.",
    ...
    e(
        ...
        "`s` with every leading code point that appears in `cutset` removed. `cutset` is a *set* of characters, not a prefix — use `TrimPrefix` for that.",
        "strings.TrimLeft(\"xxhixx\", \"x\")   // hixx",
    ),
    e(
        "strings.TrimRight",
        "Package strings",
        "strings.TrimRight(s, cutset string) string",
        "`s` with every trailing code point that appears in `cutset` removed.",
        "strings.TrimRight(\"xxhixx\", \"x\")   // xxhi",
    ),
    e(
        "strings.Compare",
        "Package strings",
        "strings.Compare(a, b string) int",
        "-1, 0 or 1 as `a` sorts before, with, or after `b`. `<` and `==` say the same thing and read better; this exists for a sort comparator.",
    ...
    e(
        ...
        "The string as a double-quoted Go literal, with the same escaping `%q` applies: a control character becomes `\\n`/`\\t`/`\\xHH`, an embedded quote is backslashed, and a non-printable rune becomes `\\uHHHH`.",
        "strconv.Quote(\"a\\\"b\\n\")   // \"a\\\"b\\n\"",
    ),
    e(
        "strconv.QuoteRune",
        ...
        "The rune as a single-quoted Go literal, escaped the same way `%q` escapes one.",
        "strconv.QuoteRune('\\t')   // '\\t'",
    ),
    e(
        "strconv.ParseBool",
        ...
        "The boolean `s` spells. Exactly ten spellings are accepted — `1 t T TRUE true True 0 f F FALSE false False` — and anything else is a `*strconv.NumError` wrapping `ErrSyntax`, with `false` as the value.",
    ...
    e(
        ...
        "`\"true\"` or `\"false\"`.",
        "strconv.FormatBool(1 == 1)   // \"true\"",
    ),
    e(
        "strconv.FormatFloat",
        "Package strconv",
        "strconv.FormatFloat(f float64, fmt byte, prec, bitSize int) string",
        "The float as text. `fmt` is one of `f` (no exponent), `e`/`E` (an exponent always), `g`/`G` (an exponent for large exponents, plain otherwise). `prec` is the digit count, or `-1` for the fewest digits that parse back to `f` exactly — at the width `bitSize` names, so `32` gives `float32`'s shorter answer. The `b`, `x` and `X` verbs (binary and hexadecimal exponents) are not implemented and fault rather than answering.",
        "strconv.FormatFloat(0.1, 'f', -1, 32)   // \"0.1\"",
    ),
